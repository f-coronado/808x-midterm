...
int main() {
  std::cout
      ...
      << "and initial joint angles or use predefined values? (y/n):"
      << std::endl;
  ...
  if (userInput == 'n') {
    ...
    std::cout << "Using EE velocity vector: \n"
              << x_dot << "\nWith initial joint angles:" << initial_angles
              << std::endl;
    Eigen::Matrix<double, 6, 6> jacobianM =
        jacobianInstance.j_matrix(0.0, 0.0, M_PI / 2, 0.0, M_PI / 2, 0.0);
    std::cout << "\n jacobian matrix is as follows: \n"
              << jacobianM << std::endl;

    // q_dot are the joint velocities we are solving for
    Eigen::Matrix<double, 6, 1> q_dot =
        robot.velocity_IK(initial_angles, x_dot, jacobianM);
    std::cout << "\nThe required joint angle velocities are:\n " << q_dot
              << std::endl;
  } else {
    std::cout
        << "Please enter 6 values for your desired end effector velocities: "
        << std::endl;
    for (int i = 0; i < 6; ++i) {
      std::cin >> x_dot[i];
    }
    ...
    std::cout
        << "\nPlease enter 6 values for your desired initial joint angles: "
        << std::endl;
    for (int i = 0; i < 6; ++i) {
